...
use crate::{datastore, models::{response::status::Status, error::AppError}, commands::{self, ping::PingCommandResult}};
...
pub async fn status_check_all() -> Result<(), AppError>  {
    ...
    for server in servers {               
        ...
        tasks.push(tokio::spawn(
            commands::execute::<PingCommandResult>(input)
        ));
...
pub async fn status_check(ips_to_check: Vec<IpAddr>, use_cache: bool) -> Result<Vec<Status>, AppError> {
    
    let list_to_check = if ips_to_check.is_empty() {
        datastore::get_all_servers().iter().map(|s| s.ipaddress).collect()
    }
    else {
        ips_to_check
    };
    ...
    let result = if use_cache {
        list_to_check.iter().map(|ipaddress| datastore::get_status(ipaddress).unwrap_or_else(|| Status::new(ipaddress.to_owned()))).collect()
    }
    else {
        Vec::new()
    };
