...
pub async fn execute<'a, R>(input: CommandInput) -> Result<R, AppError> where R: CommandResult + Clone + 'a + 'static {
    ...
    match commands.iter().find(|c| c.can_handle(&input)) {
        Some(cmd) => {
            match cmd.execute(&input).await {
                Ok(res) => {
                    let res = res.downcast::<R>().map_err(|_err| AppError::Unknown("Could not cast result to specific type".to_string()))?;
                    Ok(res.as_ref().clone())
                },
                Err(err) => {
                    log::error!("Error: {}", err);
...
impl Parameters {
    ...
    fn new(override_params: Vec<CommandArg>, params: Vec<CommandArg>, default_params: Vec<CommandArg>) -> Self {
        Parameters {
            override_params,
            params,
            default_params
...
impl CommandInput {    
    ...
    fn find_param(&self, name: &str) -> Result<&str, AppError> {
        let override_param = self.parameters.override_params.iter().find(|p| p.name == name).map(|p| p.value.as_str());

        if let Some(override_param) = override_param {
            return Ok(override_param);
        }
        let param = self.parameters.params.iter().find(|p| p.name == name).map(|p| p.value.as_str());

        if let Some(param) = param {
            return Ok(param);
        }

        self.parameters.default_params.iter().find(|p| p.name == name).map(|p| p.value.as_str())       
